...
export default function CornShop() {
  ...
  useEffect(() => {
    const fetchCornCount = async () => {
      setIsLoading(true);
      try {
        const response = await request("/api/buy", "get");
        setClients(response);
        setCornCount(response.reduce((acc:number, clientTotal:Client) => acc + clientTotal.salesCount, 0));
      } catch (error) {
        console.error("Error fetching corn count:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const buyCorn = async () => {
    ...
    try {
      console.log("Buying corn with identifier:", currentIdentifier);
      await request("/api/buy", "post",{
        identityCard:currentIdentifier.trim(),
      });
